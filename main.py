...
class MortgageCalculator(MDApp):
    ...
    def switch_screen(self, screen_name):
        ...
        credlist = self.root.ids['container']
        credlist.clear_widgets()
        for credit in credits_data:
            cred_name = str(credit['name'])  # Ensure cred_name is a string
            cred_id = credit['id']

            # Create a box layout to hold the list item and the button
            list_item_layout = BoxLayout(orientation='horizontal')

            cred_list_item = OneLineListItem(text=cred_name)
            cred_list_item.bind(on_release=lambda instance, cred_id=credit['id']: self.show_credit_details(cred_id))

            list_item_layout.add_widget(cred_list_item)

            credlist.add_widget(list_item_layout)
    ...
    def profile_show(self):
        ...
        if os.path.exists(filename):
            with open(filename, 'r', encoding='utf-8') as f:
                data = json.load(f)
                self.screen.ids.fio_full.text = data.get('fio_full', '')
                self.screen.ids.passport_data.text = data.get('passport_data', '')
                self.screen.ids.doc_inn.text = data.get('doc_inn', '')
                self.screen.ids.phone_number.text = data.get('phone_number', '')
    ...
    def get_data_by_id(self,id):
        ...
        for entry in data:
            if entry['id'] == id:
                return entry
        return None
    def show_credit_details(self,id):
        print(self.get_data_by_id(id))
        data = self.get_data_by_id(id)
        self.update_labels(data)
        self.switch_screen('credinfo')
        pass
    def save_profile(self):
        ...
        fio_full = self.screen.ids.fio_full.text
        passport_data = self.screen.ids.passport_data.text
        doc_inn = self.screen.ids.doc_inn.text
        phone_number = self.screen.ids.phone_number.text
        ...
        # Проверка существования файла и загрузка текущих данных, если они есть
        if os.path.exists(filename):
            with open(filename, 'r', encoding='utf-8') as f:
        ...
        # Обновление текущих данных новыми
        ...
        print(f'Данные сохранены: {current_data}')
    def update_labels(self, data):
        self.screen.ids.sum_property_value_result.text = f"{data['loan']}"
        self.screen.ids.sum_payment_label_result.text = f"{self.calculate_monthly_payment(data['loan'], data['months'], data['interest']):.2f}"
        self.screen.ids.sum_interest_label_result.text = f"{data['interest']}%"
        self.screen.ids.sum_overpayment_loan_label_result.text = f"{self.calculate_total_interest(data['loan'], data['months'], data['interest']):.2f}"
        self.screen.ids.sum_payments_type_label_result.text = f"{data['methode']}"
        self.screen.ids.result_title.title = f"{data['name']}"

    def calculate_monthly_payment(self, loan, months, interest):
        monthly_interest_rate = interest / 100 / 12
        if monthly_interest_rate == 0:
            return loan / months
        else:
            return loan * monthly_interest_rate / (1 - (1 + monthly_interest_rate) ** -months)

    def calculate_total_interest(self, loan, months, interest):
        total_payment = self.calculate_monthly_payment(loan, months, interest) * months
        return total_payment - loan
    ...
    def open_about_app(self, *args):
        if not self.dialog:
            self.dialog = MDDialog(
                title="Финансовый калькулятор простой и лёгкий в использовании, с помощью которого вы сможете рассчитать кредит, выбрать тип платежа, указать дату начала, месяцы, процент, а также построить диаграмму\n"
                      "giniyatov_rustem@icloud.com\n"
                      "Работу выполнил\n"
                      "студент 451 группы\n"
